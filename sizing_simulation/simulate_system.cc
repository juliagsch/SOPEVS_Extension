								  ...
								  int start_index, int end_index, double b_0, std::vector<EVRecord> evRecords, std::vector<std::vector<EVStatus>> allDailyStatuses, double max_soc, double min_soc, int Ev_start)
{
	...
	std::sort(sorted_indices.begin(), sorted_indices.end(),
			  [&annual_sums](size_t a, size_t b)
			  { return annual_sums[a] > annual_sums[b]; });

	int max_panels = static_cast<int>(solar_dataset.size());
	int trace_length = static_cast<int>(solar_dataset[0].size());

	vector<vector<double>> averaged_solar_traces;
	averaged_solar_traces.reserve(max_panels);

	// cumulative sum of the top-k traces (keeps running total as k increases)
	vector<double> cumulative(trace_length, 0.0);

	for (int k = 1; k <= max_panels; ++k)
	{
		size_t idx = sorted_indices[k - 1]; // add the next-best panel
		for (int n = 0; n < trace_length; ++n)
			cumulative[n] += solar_dataset[idx][n];

		double denom = static_cast<double>(k) * static_cast<double>(system_size_trace_pv);
		vector<double> avg(trace_length);
		for (int n = 0; n < trace_length; ++n)
			avg[n] = cumulative[n] / denom;

		averaged_solar_traces.push_back(std::move(avg));
	}
	// cout << " iteration start" << endl;
	// cout << max_panels << " " << trace_length << " " << averaged_solar_traces.size() << " " << pv_max << endl;
	// for (size_t i = 0; i < averaged_solar_traces.size(); ++i)
	// {
	// 	double sum = std::accumulate(averaged_solar_traces[i].begin(),
	// 								 averaged_solar_traces[i].end(), 0.0);
	// 	std::cout << "Top " << (i + 1) << " panels: annual sum = " << sum << '\n';
	// }
	...
	while (cells_U - cells_L > cells_step)
	{
		...
		double loss = sim(load_trace, averaged_solar_traces[ceil(pv_max / panelkW) - 1], start_index, end_index, mid_cells, pv_max, b_0, evRecords, allDailyStatuses, max_soc, min_soc, Ev_start, false);
	...
	if (cells_U == cells_max)
	{
		cout << "No valid sizing could be found. Roof is too small." << endl;
		return curve;
	}

	// cout << starting_cells * kWh_in_one_cell << " " << lowest_feasible_pv << " " << starting_cost << endl;
	...
	for (double cells = starting_cells; cells <= cells_max; cells += cells_step)
	{
		...
		if (binary_search)
		{
			...
			while (pv_U - pv_L > pv_step)
			{
				...
				// cout << ceil(mid_pv / panelkW) - 1 << " " << mid_pv << endl;
		...
		else
		{
			while (true)
			{
				...
				// cout << ceil((lowest_feasible_pv - pv_step) / panelkW) - 1 << " " << lowest_feasible_pv - pv_step << endl;
		...
		// cout << cells * kWh_in_one_cell << " " << lowest_feasible_pv << " " << cost << endl;
